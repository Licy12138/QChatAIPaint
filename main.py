...
def response_process(images, remain, generation_cost):
    ...
    msg_list += img_obj
    msg_list.append(Plain(text='剩余积分：{}\n'.format(remain)))
    msg_list.append(Plain(text='使用成本：{}'.format(generation_cost)))
    return msg_list
...
def send_msg(kwargs, msg):
    host: PluginHost = kwargs['host']
    if kwargs['launcher_type'] == 'person':
        if hasattr(host, 'send_person_message'):
            host.send_person_message(kwargs['launcher_id'], msg)
        else:
            logging.error("Host object does not have send_person_message method")
    else:
        if hasattr(host, 'send_group_message'):
            host.send_group_message(kwargs['launcher_id'], msg)
        else:
            logging.error("Host object does not have send_group_message method")
            

def send_msg(kwargs, msg):
    host: PluginHost = kwargs['host']
    logging.info(f"Attempting to send message: {msg} to {'person' if kwargs['launcher_type'] == 'person' else 'group'} with ID {kwargs['launcher_id']}")
    if kwargs['launcher_type'] == 'person':
        if hasattr(host, 'send_person_message'):
            host.send_person_message(kwargs['launcher_id'], msg)
            logging.info(f"Message sent to person {kwargs['launcher_id']}")
        else:
            logging.error("Host object does not have send_person_message method")
    else:
        if hasattr(host, 'send_group_message'):
            host.send_group_message(kwargs['launcher_id'], msg)
            logging.info(f"Message sent to group {kwargs['launcher_id']}")
        else:
            logging.error("Host object does not have send_group_message method")
            ...
            if not default_config['open']:
                ...
                try:
                    if msg.replace('aipaint', '').strip() in ['随机图片', '随机生成', '随机']:
                        ...
                        con_list = msg.replace('aipaint', '').replace('条件随机', '').strip().split('?')
                        send_msg(kwargs, ['图片生成中，请稍候'])
                        ...
                        con_list = msg.replace('aipaint', '').replace('默认生成', '').strip().split('?')
                        send_msg(kwargs, ['图片生成中，请稍候'])
                        ...
                        con_list = msg.replace('aipaint', '').replace('条件生成', '').strip().split('?')
                        send_msg(kwargs, ['图片生成中，请稍候'])
                    ...
                    msg_list = [str(ve)]
                event.add_return("reply", msg_list)
        ...
        if command == 'aipaint':
            ...
            if params[0] == 'help':
                ...
                if params[0] == 'on':
                    ...
                    if len(params) < 3:
                        event.add_return("reply", ["设置命令参数不足"])
                    else:
